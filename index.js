...
function addGamesToPage(games) {
  ...
  for (var i = 0; i < games.length; i++) {
    ...
    new_card.innerHTML = `<img src="${games[i].img}"/><h2>${games[i].name}</h2><p>${games[i].description} </p>`;
...
function showAllGames() {
  ...
  const allGames = GAMES_JSON.filter((games) => {
    return games;
  });
...
const unfundedStr = `The total number of unfunded games is: ${
  filterUnfundedOnly() ? game.pledged : "No games to show"
} `;
